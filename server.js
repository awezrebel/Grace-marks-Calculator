...
const protection = (req, res, next) => {
        ...
        console.log(token)
        if (token) {
          jwt.verify(token, "pavan", (err, decodedToken) => {
            if (err) {
              console.log(err);
...
const createToken= (id)=>{
        return jwt.sign({id},"pavan",{expiresIn: maxAge })
    }
...
app.get("/", (req, res) => {
  ...
  app.get("/Faculty", function (req, res) {
    ...
    result = {
      Name: "Faculty",
      image: "http://localhost:5000/img/Faculty.png",
    };
    obj = { welcome: result };
    res.render("welcome", obj);
  });
  ...
  app.get("/Student", function (req, res) {
    ...
    result = {
      Name: "Student",
      image: "http://localhost:5000/img/student.png",
    };
    obj = { welcome: result };
    res.render("welcome", obj);
    return;
  ...
  app.get("/Coordinator", function (req, res) {
    ...
    result = {
      Name: "Co ordinator",
      image: "http://localhost:5000/img/co.png",
    };
    obj = { welcome: result };
    res.render("welcome", obj);
  });
  ...
  if (otp_user != null) {
    ...
    console.log(
      "user " + uname1 + "  otp user got = " + otp_user + " acknowledge " + ack
    );
  ...
  if (issue != null && section != null) {
    console.log("dssssssss" + issue + section);
    mycon.query(
      ` INSERT INTO database1.classissues (section, issue) VALUES ('${section}', '${issue}');`,
      function (err, result) {
        if (err) {
          throw err;
        }
  ...
  app.get("/loginwelcome", function (req2, res2) {
    fs.readFile("logintype.txt", "utf-8", (err, data1) => {
      fs.readFile("currentlogin.txt", "utf-8", (err, data) => {
        ...
        mycon.query(
          `SELECT name FROM ${data1}  WHERE (uname ='${data}')`,
          function (err, result) {
            if (err) {
              throw err;
            } else {
    ...
    app.get("/data", function (req, res) {
      fs.readFile("logintype.txt", "utf-8", (err, data1) => {
        fs.readFile("currentlogin.txt", "utf-8", (err, data) => {
          ...
          mycon.query(
            `SELECT * FROM marks , profile where  ( profile.rollno = '${data}') `,
            function (err, result) {
              if (err) {
                throw err;
              } else {
    ...
    app.get("/faculty_details", function (req, res) {
      var obj2 = {};
      mycon.query(
        ` SELECT * FROM database1.profile where type="faculty"; `,
        function (err, result) {
          if (err) {
            throw err;
          } else {
    ...
    app.get("/class_issues", function (req, res) {
      var obj2 = {};
      mycon.query(
        ` SELECT * FROM database1.classissues; `,
        function (err, result) {
          if (err) {
            throw err;
          } else {
    ...
    app.get("/tracking", function (req, res) {
      var obj2 = {};
      mycon.query(
        ` SELECT * FROM database1.tracking; `,
        function (err, result) {
          if (err) {
            throw err;
          } else {
...
main
    ...
    app.get("/attendance", function (req, res) {
      mycon.query(
        `SELECT * FROM database1.attendance; `,
        function (err, result) {
          if (err) {
            throw err;
          } else {
    ...
    app.get("/grading", function (req, res) {
      mycon.query(
        `SELECT * FROM database1.gradingscheme; `,
        function (err, result) {
          if (err) {
            throw err;
          } else {
    ...
    app.get("/uploading",protection,function (req, res) {
      var spawn = require("child_process").spawn;
